...
import { AlertCircle, Download, Trash2, Plus } from "lucide-react"
            ...
            <CardContent className="space-y-6">
              <div className="grid md:grid-cols-2 gap-6">
                <div className="space-y-3">
                  ...
                  <Button
                    ...
                    disabled={!jobDesc.trim() || !jobTitle.trim()}
                    variant="outline"
                    size="sm"
                ...
                <div className="space-y-3">
                  ...
                  <Input
                    placeholder="Resume name (e.g., Senior Dev Resume)"
                    value={resumeName}
              ...
              <div className="p-4 rounded-lg border bg-card">
                ...
                {extractedSkills.length === 0 ? (
                  <div className="text-sm text-muted-foreground">No recognized skills detected in resume.</div>
                ) : (
                  <div className="flex flex-wrap gap-2">
        ...
        <TabsContent value="resumes">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Manage and quickly load your saved resumes for analysis.</CardDescription>
            </CardHeader>
            <CardContent>
              {savedResumes.length === 0 ? (
                <Alert>
                  ...
                  <AlertDescription>
                    No saved resumes yet. Save your first resume from the Analyzer tab.
                  </AlertDescription>
                </Alert>
              ) : (
                <div className="space-y-3">
                  {savedResumes.map((savedResume) => (
                    ...
                    >
                      <div className="flex-1">
                        <div className="font-medium text-sm">{savedResume.name}</div>
                        <div className="text-xs text-muted-foreground">Saved on {savedResume.createdAt}</div>
                      </div>
                      <div className="flex gap-2">
                        <Button onClick={() => handleLoadResume(savedResume)} variant="outline" size="sm">
        ...
        <TabsContent value="jobs">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Keep track of job descriptions you want to analyze against.</CardDescription>
            </CardHeader>
            <CardContent>
              {savedJobs.length === 0 ? (
                <Alert>
                  <AlertCircle className="h-4 w-4" />
                  <AlertDescription>No saved jobs yet. Save your first job from the Analyzer tab.</AlertDescription>
                </Alert>
              ) : (
                <div className="space-y-3">
                  {savedJobs.map((savedJob) => (
                    ...
                    >
                      <div className="flex-1">
                        <div className="font-medium text-sm">{savedJob.title}</div>
                        <div className="text-xs text-muted-foreground">Saved on {savedJob.createdAt}</div>
                      </div>
                      <div className="flex gap-2">
                        <Button onClick={() => handleLoadJob(savedJob)} variant="outline" size="sm">
                          <Download className="w-4 h-4 mr-1" />
                          Load
                        </Button>
                        <Button
